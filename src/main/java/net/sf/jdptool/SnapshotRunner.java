    ...
    public void run() {
        ...
        try {
            ...
            while (!monitor.vmDied && this.interval > 0) {
                ...
                Collection<ReferenceType> classes = CollectionUtils.select(monitor.vm.allClasses(),
                                                            new NotExcludesPredicate(defaultExcludes));
                for (Iterator<ReferenceType> it = classes.iterator(); it!=null && it.hasNext(); ) {
                    ReferenceType clazz = it.next();
